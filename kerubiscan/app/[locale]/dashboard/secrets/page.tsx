...
export default function SecretsPage() {
  ...
  const columns = [
    ...
    { header: "Type", accessor: "credential_type" as const },
    { header: "Asset", accessor: (row: any) => getAssetName(row.asset_id), className: "text-text-muted" },
    { header: "Updated On", accessor: (row: any) => new Date(row.updated_at).toLocaleDateString(), className: "text-text-muted" },
    {
      header: "Actions",
      accessor: (row: any) => (
        <button 
  ...
  return (
    <div className="pb-6">
      <PageHeader 
        ...
        description="Manage encrypted credentials for authenticated scans" 
        action={
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-surface border border-border rounded-xl w-full max-w-lg shadow-2xl flex flex-col">
            <div className="flex items-center justify-between p-5 border-b border-border/50">
              <h3 className="font-semibold text-lg">Add New Secret</h3>
            ...
            <form onSubmit={handleAddSecret} className="p-5 flex flex-col gap-4">
              <div>
                <label className="block text-sm font-medium text-text-muted mb-1.5">Secret Name</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={newSecret.type} onChange={e => setNewSecret({...newSecret, type: e.target.value})} className="w-full bg-base border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary transition-colors appearance-none">
                    ...
                    <option value="smb">SMB Password</option>
              ...
              <div>
                <label className="block text-sm font-medium text-text-muted mb-1.5">
                  {newSecret.type === "ssh" ? "Private Key" : newSecret.type === "snmpv2" ? "Community String" : newSecret.type === "aws" ? "Secret Access Key" : "Password"}
                </label>
              ...
              <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-border/50">
                ...
                <button type="submit" className="px-4 py-2 bg-primary hover:bg-primary-hover text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
                  <Lock className="w-4 h-4" /> Save Securely
                </button>
              </div>
            </form>
          </div>
